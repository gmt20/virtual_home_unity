...
namespace StoryGenerator.Recording
{
    ...
    public class Recorder : MonoBehaviour
    {
                                ...
                                foreach (var sensor in MotionSensorManager.GetAllSensors())
                                {
                                    if (sensor.IsPositionDetected(position.Value))
                                    {
                                        motionEvents.Add(new MotionDetectionEvent
                                        {
                                            ...
                                            characterPositionX = position.Value.x,
                                            characterPositionY = position.Value.y,
                                            characterPositionZ = position.Value.z
                                        });
                                    }
                                }
                            }
        ...
        public void CreateTextualGTs()
        {
            ...
            const string PREFIX_MOTION_EVENTS = "motion_"; // Add prefix for trajectory data
            ...
            String motionEventsFileName = Path.Combine(OutputDirectory, PREFIX_MOTION_EVENTS) + FileName + FILE_EXT_TXT;
            ...
            if (motionEvents.Count == 0) {
                ...
                using (StreamWriter sw = new StreamWriter(motionEventsFileName)) {
                    foreach (var evt in motionEvents) {
                        sw.WriteLine($"{evt.frameNumber} {evt.characterId} {evt.sensorId} {evt.roomName} {evt.characterPositionX} {evt.characterPositionY} {evt.characterPositionZ}");
                    }
                }
            }
