...
namespace StoryGenerator
{
    ...
    public class TestDriver : MonoBehaviour
    {
        ...
        private string logPath = "/home/yusuf/Desktop/megha/vh_builds/sensor_placement_log.txt";
        ...
        IEnumerator ProcessNetworkRequest()
        {
            ...
            while (true) {
                ...
                else if (networkRequest.action == "add_sensors") 
                {
                    if (networkRequest.intParams?.Count > 0)
                    {
                        int environmentId = networkRequest.intParams[0];
                       
                        
                        if (houseTransform == null)
                        {
                            response.success = false;
                            response.message ="houseTransform is null";
                        ...
                        else
                        {
                            ...
                            try 
                            {
                                MotionSensorPlacer sensorPlacer = new MotionSensorPlacer(houseTransform, environmentId.ToString());
                              
                                var placedSensors = sensorPlacer.PlaceSensorsInEnvironment();
                                response.success = true;
                                response.message = JsonConvert.SerializeObject(placedSensors);
                                
                            }
                            catch (Exception ex)
                            {
                                response.success = false;
                                response.message = $"catch Error placing sensors: {ex.Message}";
                            }
                        }
                    }
                    else
                    {
                        response.success = false;
                        response.message = "No environment Id provided";
                    }
                } 
                ...
                // {
                //     string logMessage = $"[{DateTime.Now}] Entering place_sensors action\n";
                ...
                //     response.message = "";
                    ...
                    // {
                    //     int environmentId = networkRequest.intParams[0];
                    //     logMessage = $"[{DateTime.Now}] Environment ID: {environmentId}\n";
                    //     File.AppendAllText(logPath, logMessage);
                    //     Debug.Log(logMessage); // This will still show in Unity Console
                        
                    //     if (houseTransform == null)
                    //     {
                    //         response.success = false;
                    //         response.message ="houseTransform is null";
                    //         logMessage = $"[{DateTime.Now}] Error: houseTransform is null\n";
                    //         File.AppendAllText(logPath, logMessage);
                    //     }
                    //     else
                    //     {
                    ...
                    //         {
                    //             MotionSensorPlacer sensorPlacer = new MotionSensorPlacer(houseTransform, environmentId.ToString());
                    //             logMessage = $"[{DateTime.Now}] Created MotionSensorPlacer for environment {environmentId}\n";
                    //             File.AppendAllText(logPath, logMessage);
                                
                    //             var (placedSensors, errorMsg) = sensorPlacer.PlaceSensorsInEnvironment();
                                
                    //             if (errorMsg != null)
                    //             {
                    //                 response.success = false;
                    //                 response.message = errorMsg;
                    //                 logMessage = $"[{DateTime.Now}] Error placing sensors: {errorMsg}\n";
                    //                 File.AppendAllText(logPath, logMessage);
                    //             }
                    //             else
                    //             {
                    //                 response.success = true;
                    //                 response.message = JsonConvert.SerializeObject(placedSensors);
                    //                 logMessage = $"[{DateTime.Now}] Successfully placed {placedSensors?.Count ?? 0} sensors\n";
                    //                 File.AppendAllText(logPath, logMessage);
                    //             }
                    //         }
                    //         catch (Exception ex)
                    //         {
                    //             logMessage = $"[{DateTime.Now}] Exception during sensor placement: {ex.Message}\n{ex.StackTrace}\n";
                    //             File.AppendAllText(logPath, logMessage);
                    //             response.success = false;
                    ...
                    // else
                    // {
                    //     logMessage = $"[{DateTime.Now}] Error: No environment ID provided\n";
                    //     File.AppendAllText(logPath, logMessage);
                    //     response.success = false;
                    //     response.message = "No environment Id provided";
                    // }
